...
min_values = np.min(data[:,:,:-1].sum(axis=1), axis=1)/N
max_values = np.max(data[:,:,:-1].sum(axis=1), axis=1)/N
# 固定图形大小
figsize = (10, 6)
# 绘制预测值
...
plt.plot(time, data[:,:,:-1].sum(axis=1).mean(axis=1)/N, label='Average',lw=1,color='darkblue')
plt.plot(time, data[:,:,-1].sum(axis=1)/N, label='real',lw=1,color='red')

plt.ylim(1, 5)
plt.legend(loc='upper left', bbox_to_anchor=(1, 1), borderaxespad=0.)
plt.xticks(range(1993, 2013, 2))
plt.title('1993——2012年观测值、简单平均及最大最小值阴影、四种GBRT方法留一验证折线图')
plt.tight_layout()
plt.savefig('./fig2.1.png', dpi=1000, bbox_inches='tight')
plt.show()
plt.close()
